...
@section('app-content')
    ...
    <section class=" max-w-3xl mx-auto p-8 flex flex-wrap justify-center gap-3">
        ...
        @foreach ($viewData['orders'] as $order)
            <a href="{{ route('ordersInfo', [ $sessionInfo->table_number, $sessionInfo->session_token, $order->id])}}">
                <div class=" w-80 h-32 rounded-2xl bg-gray-50 shadow-2xs grid grid-cols-[100px_auto] py-3 px-2 overflow-hidden items-center">
                    <img src="
                    {{ $order->status=='cancelled' ? 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR2f0_e4yGqNqCirPNlWpCTX1_gYRdwzI9mrjapBbb-oiWErUvW-BDz7uQ&s=10' : 'https://uxwing.com/wp-content/themes/uxwing/download/e-commerce-currency-shopping/order-placed-purchased-icon.svg'}}
                    " alt="" class=" mix-blend-multiply scale-75">
                    <div class=" px-2 py-3 flex flex-col gap-1.5 ">
                        ...
                        <span class=" w-fit px-1 py-0.5 font-medium text-xs capitalize rubik rounded-sm 
                        {{ $order->status== 'cancelled' ? 'bg-red-100 text-red-500' : 'bg-green-100 text-green-500'}}
                        ">{{ $order->status}}</span>
                        <span class=" text-xs font-medium inter">View >></span>
                    </div>
                </div>
            </a>
        @endforeach
